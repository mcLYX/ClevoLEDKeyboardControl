...
internal sealed class AudioBandLevelMeter : IDisposable
{
    ...
    /// 每个 Goertzel 频点只算一次，同时累加原始与加权，避免重复计算。
    /// </summary>
    public TriBandAnalysis GetTriBandAnalysis()
    {
        EnsureCapture();
        var snapshot = AcquireSnapshot(out var sampleRate, out var snapshotAcquired);
        try
        {
            if (snapshot.Length < 256)
            {
                return new TriBandAnalysis(
                    new TriBandLevels(0, 0, 0),
                    new TriBandLevels(0, 0, 0));
            }

            var (rawLow, weightedLow) = GetBandLevelDual(snapshot, sampleRate, 35, 280);
            var (rawMid, weightedMid) = GetBandLevelDual(snapshot, sampleRate, 280, 2500);
            var (rawHigh, weightedHigh) = GetBandLevelDual(snapshot, sampleRate, 2500, 16000);
            return new TriBandAnalysis(
                new TriBandLevels(rawLow, rawMid, rawHigh),
                new TriBandLevels(weightedLow, weightedMid, weightedHigh));
        }
        finally
        {
            if (snapshotAcquired)
    ...
    /// <summary>
    /// A-weighting 幅度域增益（IEC 61672-1）。返回 10^(A(f)/20)，可直接乘到幅度上。
    /// 1kHz 处增益为 1.0；低频显著衰减（100Hz≈0.11），2-6kHz 略提升，极高频再衰减。
    /// </summary>
    private static double AWeightingGain(double frequencyHz)
    {
        if (frequencyHz <= 0)
        {
            return 0;
        ...
        const double normalization = 1.2589; // 10^(2.00/20)，使 1kHz 处增益=1

        var f2sq = frequencyHz * frequencyHz;
        var numerator = f4 * f4 * f2sq * f2sq;
        var denominator = (f2sq + f1 * f1)
    ...
    /// <summary>
    /// 一次循环同时计算频段的原始能量与 A-weighting 加权能量。
    /// Goertzel 每个频点只算一次，原始累加 energy，加权累加 energy*gain²。
    /// </summary>
    private static (double raw, double weighted) GetBandLevelDual(ReadOnlySpan<float> samples, int sampleRate, int lowHz, int highHz)
    {
        lowHz = Math.Clamp(lowHz, 20, sampleRate / 2 - 20);
        highHz = Math.Clamp(highHz, lowHz + 10, sampleRate / 2);
        var rawTotal = 0d;
        ...
        for (var hz = lowHz; hz <= highHz; hz += Math.Max(10, (highHz - lowHz) / 8))
        {
            var energy = Goertzel(samples, sampleRate, hz);
            var gain = AWeightingGain(hz);
            rawTotal += energy;
            // Goertzel 返回能量（幅度平方），A-weighting 是幅度域增益，能量域需乘 gain²
            weightedTotal += energy * gain * gain;
            count++;
        }
        ...
        var norm = Math.Max(1, samples.Length / 2d);
