...
本仓库的更改全是 vibe 的，仅在无分区背光的机型上测试，不作任何担保。
...
- **频率分布模式**：新增基于实时三频能量比例的颜色映射模式，不做节拍检测，颜色连续变化；支持人耳等响度补偿（A-weighting）和动态范围监测（峰值/下限跟踪），适应不同响度和动态范围的音源。
- **渐变循环灯效**：在颜色序列中平滑过渡，支持 RGB 线性插值和 HSV 空间插值。
- **全局亮度 Gamma 校正**：高级选项可调 0.8~3.0，影响所有灯效的亮度映射曲线。
=======
...
- 渐变循环：在颜色序列中平滑过渡，支持 RGB 线性插值和 HSV 空间插值，可配置停留时长、过渡时长和过渡亮度。
- 全局亮度 Gamma 校正：高级选项卡可调 0.8~3.0，影响所有灯效的亮度映射曲线。
=======
- 敲字闪烁：输入时短暂提高键盘亮度。
...
- 支持三种响应模式：
  - **按电平变色**：节拍驱动，每次节拍切换颜色。
  - **仅亮度脉冲**：固定单个颜色，只有亮度随节拍脉冲。
  - **频率分布**：根据实时三频（低/中/高）能量比例映射颜色，颜色连续变化，不做节拍检测。可调整基础亮度、峰值亮度和色调偏移。
- 频率分布模式支持人耳等响度补偿（A-weighting，IEC 61672-1），避免低频能量压制中高频。
- 频率分布模式支持动态范围监测（不启用 / 仅峰值 / 峰值与下限），适应不同响度音源和“响度战争”压缩音源。
=======
- 支持节拍脉冲、多色列表和自适应鼓点检测。
