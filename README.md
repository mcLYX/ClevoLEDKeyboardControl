...
程序通过 `InsydeDCHU.dll` 调用厂商接口控制键盘灯。安装器会优先从 `C:\Program Files (x86)\ControlCenter` 自动复制该 DLL（来自 Clevo / 蓝天厂商提供的 Control Center 软件）。如未安装 Control Center，请参见下方“获取驱动 DLL”章节手动准备。
...
- 音乐模式支持按电平变色和仅亮度脉冲，并提供自适应鼓点检测。
- 音乐模式内置自定义、流行、摇滚、电子预设，并支持最多 8 个命名自定义预设。
- 音乐模式支持按 Windows 通用媒体会话或 Spotify OAuth 获取专辑封面主题色，用作单色亮度脉冲基础色。
- 支持敲字闪烁：输入时短暂提高键盘亮度。
...
`InsydeDCHU.dll` 是 Clevo / 蓝天厂商提供的私有驱动，**不随本仓库分发**，需要你自行准备：

- **首选**：在你的笔记本上安装厂商的 Control Center / Control Center 3.0 软件，安装器会自动从 `C:\Program Files (x86)\ControlCenter\InsydeDCHU.dll` 复制。
- **备选**：手动将 `InsydeDCHU.dll` 放到下列任一位置后再运行安装器或服务：
  - `C:\Program Files\ClevoLEDKeyboardControl\Service\InsydeDCHU.dll`
  - 源码构建场景：`assets\driver\InsydeDCHU.dll`（构建脚本会一并拷入安装包）。

如果两条路径都找不到 DLL，安装可以完成，但服务无法控制键盘灯，托盘设置页的诊断区会提示驱动缺失。
