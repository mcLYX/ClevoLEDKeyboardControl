...
<<<<<<< HEAD
    ...
    private readonly ColorPickerRow _musicPulseColor = new("脉冲颜色");
    private readonly SliderRow _musicFreqHue = new("色调偏移", 0, 360, "°");
    private readonly CheckBox _musicEqualLoudness = new() { Text = "人耳等响度补偿（A-weighting）" };
    private readonly ComboBox _musicDynamicRange = new()
    {
...
<<<<<<< HEAD
        ...
        page.Controls.Add(_musicColorsSection);
        page.Controls.Add(_musicSequence);
        page.Controls.Add(_musicPulseColor);
...
<<<<<<< HEAD
            _musicResponseMode.SelectedIndex = settings.Effect.Music.ResponseMode switch
            {
                ...
                MusicResponseMode.FrequencyDistribution => 2,
                _ => 0
            };
            _musicSequence.Colors = settings.Effect.Music.Colors;
            _musicPulseColor.ColorHex = settings.Effect.Music.Colors.Count > 0 ? settings.Effect.Music.Colors[0] : "#FF0000";
            _musicFreqHue.Value = settings.Effect.Music.FreqHueOffset;
            _musicEqualLoudness.Checked = settings.Effect.Music.EqualLoudness;
            _musicDynamicRange.SelectedIndex = settings.Effect.Music.DynamicRange switch
            {
                DynamicRangeMode.None => 0,
                DynamicRangeMode.PeakOnly => 1,
...
<<<<<<< HEAD
            if (selectedEffect == EffectType.GradientCycle)
            {
                ...
                settings.Effect.GradientTransitionMs = _gradientTransition.Value;
...
<<<<<<< HEAD
            settings.Effect.Music.ResponseMode = _musicResponseMode.SelectedIndex switch
            {
                ...
                2 => MusicResponseMode.FrequencyDistribution,
                _ => MusicResponseMode.LevelColor
            };
            settings.Effect.Music.LevelColorEnabled = settings.Effect.Music.ResponseMode == MusicResponseMode.LevelColor;
            // BrightnessPulse 模式用单色选择器的颜色
            settings.Effect.Music.Colors = settings.Effect.Music.ResponseMode == MusicResponseMode.BrightnessPulse
                && !string.IsNullOrWhiteSpace(_musicPulseColor.ColorHex)
                ? [_musicPulseColor.ColorHex]
                : NormalizedMusicColors();
            ...
            settings.Effect.Music.FreqHueOffset = _musicFreqHue.Value;
            settings.Effect.Music.EqualLoudness = _musicEqualLoudness.Checked;
            settings.Effect.Music.DynamicRange = _musicDynamicRange.SelectedIndex switch
            {
                0 => DynamicRangeMode.None,
...
<<<<<<< HEAD
        if (left.Type == EffectType.GradientCycle)
        {
            if (left.GradientHoldMs != right.GradientHoldMs ||
                left.GradientTransitionMs != right.GradientTransitionMs ||
                left.GradientMinBrightnessPercent != right.GradientMinBrightnessPercent ||
                left.GradientAlgorithm != right.GradientAlgorithm)
            {
                return false;
            }
        }
...
<<<<<<< HEAD
        if (effectType == EffectType.GradientCycle)
        {
            ...
            effect.GradientTransitionMs = _gradientTransition.Value;
            effect.GradientMinBrightnessPercent = _gradientMinBrightness.Value;
...
<<<<<<< HEAD
        ...
        _musicPulseColor.ColorHex = preset.Colors.Count > 0 ? preset.Colors[0] : "#FF0000";
        _musicFreqHue.Value = preset.FreqHueOffset;
        _musicEqualLoudness.Checked = preset.EqualLoudness;
        _musicDynamicRange.SelectedIndex = preset.DynamicRange switch
        {
            DynamicRangeMode.None => 0,
            DynamicRangeMode.PeakOnly => 1,
            DynamicRangeMode.PeakAndFloor => 2,
...
<<<<<<< HEAD
        ...
        // BrightnessPulse 模式用单色选择器的颜色
        if (responseMode == MusicResponseMode.BrightnessPulse && !string.IsNullOrWhiteSpace(_musicPulseColor.ColorHex))
        {
            colors = [_musicPulseColor.ColorHex];
        }
        return new MusicPreset
        {
            ...
            HighColor = colors[^1],
            Colors = colors,
            FreqHueOffset = _musicFreqHue.Value,
            EqualLoudness = _musicEqualLoudness.Checked,
            DynamicRange = _musicDynamicRange.SelectedIndex switch
            {
                0 => DynamicRangeMode.None,
                2 => DynamicRangeMode.PeakAndFloor,
                _ => DynamicRangeMode.PeakOnly
...
<<<<<<< HEAD
    ...
    private void UpdateMusicResponseModeVisibility()
    {
        var mode = _musicResponseMode.SelectedIndex;
        var levelColor = mode == 0;
        var brightnessPulse = mode == 1;
        var freqDist = mode == 2;

        // 颜色相关控件
        if (_musicColorsSection is not null) _musicColorsSection.Visible = levelColor;
        _musicSequence.Visible = levelColor;
        if (_musicCustomColorsRow is not null) _musicCustomColorsRow.Visible = levelColor;
        _musicPulseColor.Visible = brightnessPulse;
        _musicFreqHue.Visible = freqDist;
        _musicEqualLoudness.Visible = freqDist;
        _musicDynamicRange.Visible = freqDist;
